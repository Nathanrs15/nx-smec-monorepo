  ...
  template: `
    <ng-container *ngIf="data">
      <form
        ...
        (ngSubmit)="onSubmit()"
      >
        <p class="text-lg font-medium">Permisos para {{ resourceName }}</p>
        <p class="text-secondary mb-6">
          Asigna los permisos que un rol tiene a un determinado recurso.
        </p>
        <div class="flex flex-col gt-xs:flex-row">
          <ng-container *ngFor="let item of data.roleClaims">
            <div class="flex-auto gt-xs:pr-3 grid grid-cols-1 gap-4 w-full mt-4">
